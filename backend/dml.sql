...
CREATE OR REPLACE FUNCTION create_item(_user_id INTEGER, _name VARCHAR, _price DECIMAL, _url VARCHAR, _source_url VARCHAR)
RETURNS INTEGER AS $$
DECLARE _id INTEGER;
BEGIN
  INSERT INTO Items (user_id, name, price, url, source_url)
  VALUES (_user_id, _name, _price, _url, _source_url)
  RETURNING id INTO _id;
  RETURN _id;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TABLE(id INTEGER, user_id INTEGER, name VARCHAR, price DECIMAL, date_added TIMESTAMP, url VARCHAR, source_url VARCHAR) AS $$
BEGIN
  RETURN QUERY
  SELECT id, user_id, name, price, date_added, url, source_url
